...
export async function POST(req) {
  ...
  try {
    ...
    const fbId = await saveAccount(userId, {
      platform: 'facebook', name: page.name,
      platform_user_id: page.id, page_id: page.id, page_name: page.name,
      access_token: page.access_token, avatar: null,
    })
    if (page.instagram_business_account) {
      const igId = await saveAccount(userId, {
        platform: 'instagram',
